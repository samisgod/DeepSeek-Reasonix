...
type notifyIconData struct {
	...
	Info                       [256]uint16
	TimeoutOrVersion           uint32 // uTimeout and uVersion share one native union.
	InfoTitle                  [64]uint16
	InfoFlags                  uint32
...
// SetIconID configures a stable Windows notification-area identity. It must be
// called before Run so every add, modify, delete, and Explorer restart uses the
// same GUID.
func SetIconID(id string) error {
	guid, err := windows.GUIDFromString(id)
	if err != nil {
		return fmt.Errorf("invalid tray icon GUID: %w", err)
	}
	wt.muNID.Lock()
	defer wt.muNID.Unlock()
	if wt.initialized.Load() || wt.nid != nil {
		return fmt.Errorf("tray icon GUID must be set before Run")
	}
	wt.iconGUID = guid
	wt.useIconGUID = true
	return nil
