...
// path; measured 0.4 KiB gzip for the same block.
const initialJSBudgetKiB = 469.7;
...
for (const path of localeChunks) {
  ...
  // The progress-budget settings add their labels and validation hints per
  // dialect (~0.4 KiB gzip measured on the upstream base); retain the same
  // headroom here.
  ...
  const budget = name.startsWith("zh-TW-") ? 64.4 * 1024 : 63.7 * 1024;
