// Ported from DeepSeek Harness c291e7961a (MIT); host Markdown and safe links are retained.
...
import css from './WebBlock.styles'
...
function SafeLink({ url, label, className }: { url: string; label: string; className?: string | undefined }) {
  ...
  return (
    <span className={className}><RichMarkdownLink href={href}>
      <Globe className={css.linkIcon} />
      {label}
    </RichMarkdownLink></span>
  )
}
  ...
  return (
    <div className={clsx(css.block, className)} data-web="search">
      {answer !== undefined && answer !== '' && (
        <div className={css.answer}><Markdown text={answer} /></div>
      )}
      {empty ? (
