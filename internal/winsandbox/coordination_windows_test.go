...
func TestWindowsRootLockNamesAreSortedAndDeduped(t *testing.T) {
	...
	if len(names) < 2 {
		t.Fatalf("lock names = %v, want roots and their ancestor domains", names)
	}
	...
	if len(same) == 0 {
		t.Fatal("single root produced no lock domains")
	}
	...
	if len(windowsRootLockNames([]string{`C:\work\a`, `C:\WORK\A`})) != len(same) {
		t.Fatalf("case-insensitive dedup broken: %v vs %v", names, same)
	}
}

func TestWindowsRootLockParentAndChildShareDomain(t *testing.T) {
	parent := windowsRootLockNames([]string{`C:\work\repo`})
	child := windowsRootLockNames([]string{`C:\work\repo\secret`})
	shared := false
	...
	if !shared {
		t.Fatalf("parent and child locks do not conflict: parent=%v child=%v", parent, child)
	}
}
...
func TestWindowsSandboxConcurrentWritesToSharedWorkspace(t *testing.T) {
	...
	// The deterministic capability ACE intentionally remains. Broad AppContainer
	// grants and temporary current-user deny ACEs must not remain.
